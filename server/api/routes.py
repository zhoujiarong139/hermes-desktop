...
def _dlog(hypothesisId: str, location: str, message: str, data: dict | None = None, runId: str = "pre-fix"):
    """NDJSON debug log for Cursor Debug mode (no secrets)."""
    try:
        rec = {
            "sessionId": "652fbf",
            "runId": runId,
        ...
        with open("/Users/zhoujiarong/Documents/nous-hermes/.cursor/debug-652fbf.log", "a", encoding="utf-8") as f:
...
from api.chat_files import (
    handle_list_chat_files,
    handle_get_chat_file,
    handle_serve_chat_file,
    handle_add_chat_file,
    handle_add_to_assets,
    handle_remove_chat_file,
    handle_forward_chat_file,
)
...
def handle_get(handler, parsed) -> bool:
    ...
    if parsed.path == "/api/billing/plans":
        from api.billing import get_plans

        return j(handler, {"plans": get_plans()})

    if parsed.path == "/api/billing/subscription":
        from api.billing import get_subscription_with_plan, get_default_user_id

        user_id = get_default_user_id()
        return j(handler, get_subscription_with_plan(user_id))

    if parsed.path == "/api/billing/usage":
        from api.billing import get_user_usage, get_default_user_id

        user_id = get_default_user_id()
        return j(handler, get_user_usage(user_id))

    if parsed.path == "/api/billing/check_limits":
        from api.billing import check_limits, get_default_user_id

        user_id = get_default_user_id()
        return j(handler, check_limits(user_id))

    if parsed.path == "/api/billing/payment_status":
        from api.billing import get_payment_status
        qs = parse_qs(parsed.query)
        order_id = qs.get("order_id", [""])[0]
        if not order_id:
            return bad(handler, "order_id required")
        return j(handler, get_payment_status(order_id))

    if parsed.path.startswith("/api/billing/qr_code"):
        from api.billing import generate_qr_code
        qs = parse_qs(parsed.query)
        order_id = qs.get("order_id", [""])[0]
        if not order_id:
            return bad(handler, "order_id required", 400)
        svg_data = generate_qr_code(order_id)
        if not svg_data:
            return bad(handler, "Order not found", 404)
        handler.send_response(200)
        handler.send_header("Content-Type", "image/svg+xml")
        handler.send_header("Cache-Control", "no-store")
        handler.end_headers()
        handler.wfile.write(svg_data)
        return True

    # ── Recharge API (GET) ──
    if parsed.path == "/api/billing/recharge/order_status":
        from api.billing import get_recharge_status
        qs = parse_qs(parsed.query)
        order_id = qs.get("order_id", [""])[0]
        if not order_id:
            return bad(handler, "order_id required")
        return j(handler, get_recharge_status(order_id))

    if parsed.path == "/api/billing/balance":
        from api.billing import get_user_balance, get_default_user_id
        user_id = get_default_user_id()
        return j(handler, get_user_balance(user_id))

    # ── Asset Library (GET) ────────────────────────────────────────────────────

    if parsed.path == "/api/assets":
        return _handle_list_assets(handler)

    if parsed.path.startswith("/api/assets/"):
        # e.g. /api/assets/icon.png
        asset_name = parsed.path[len("/api/assets/"):]
        return _handle_get_asset(handler, asset_name)

    if parsed.path == "/api/workspace/files":
        return _handle_list_workspace_files(handler, parsed)

    # ── Chat Task Files ──────────────────────────────────────────────────────
    if parsed.path == "/api/chat-files":
        return handle_list_chat_files(handler)

    if parsed.path.startswith("/api/chat-files/"):
        # /api/chat-files/{id}/content → serve file bytes
        if parsed.path.endswith("/content"):
            file_id = parsed.path[len("/api/chat-files/") : -len("/content")]
            return handle_serve_chat_file(handler, file_id)
        # /api/chat-files/{id} → get file metadata
        file_id = parsed.path[len("/api/chat-files/") :]
        return handle_get_chat_file(handler, file_id)
...
def handle_post(handler, parsed) -> bool:
    ...
    # ── Gitea (GET/POST) ──
    if parsed.path == "/api/gitea":
        if handler.command == "GET":
            from api.config import load_gitea_settings
            return j(handler, load_gitea_settings())
        else:
            return _handle_gitea_save(handler, body)

    if parsed.path == "/api/gitea/repos":
        return _handle_gitea_repos(handler)

    if parsed.path == "/api/gitea/clone":
        return _handle_gitea_clone(handler, body)
    ...
    if parsed.path == "/api/settings/smtp-test":
        import smtplib
        ...
        from_addr = body.get("smtp_from", user)
        if not host or not user:
            return bad(handler, "host and user required", 400)
        try:
            ...
            return bad(handler, "Authentication failed. Check your credentials.", 400)
        except smtplib.SMTPException as e:
            return bad(handler, f"SMTP error: {e}", 400)
    ...
    # ── Billing API (POST) ──
    if parsed.path == "/api/billing/subscribe":
        from api.billing import create_subscription, get_default_user_id
        try:
            require(body, "plan_id")
        except ValueError as e:
            return bad(handler, str(e))
        user_id = get_default_user_id()
        billing_cycle = body.get("billing_cycle", "yearly")
        try:
            subscription = create_subscription(user_id, body["plan_id"], billing_cycle)
            return j(handler, {"ok": True, "subscription": subscription})
        except ValueError as e:
            return bad(handler, str(e))

    if parsed.path == "/api/billing/cancel":
        from api.billing import cancel_subscription, get_default_user_id
        user_id = get_default_user_id()
        try:
            subscription = cancel_subscription(user_id)
            return j(handler, {"ok": True, "subscription": subscription})
        except ValueError as e:
            return bad(handler, str(e))

    if parsed.path == "/api/billing/renew":
        from api.billing import renew_subscription, get_default_user_id
        user_id = get_default_user_id()
        try:
            subscription = renew_subscription(user_id)
            return j(handler, {"ok": True, "subscription": subscription})
        except ValueError as e:
            return bad(handler, str(e))

    if parsed.path == "/api/billing/upgrade":
        from api.billing import upgrade_subscription, get_default_user_id
        try:
            require(body, "plan_id")
        except ValueError as e:
            return bad(handler, str(e))
        ...
        try:
            subscription, proration = upgrade_subscription(user_id, body["plan_id"], billing_cycle)
            return j(handler, {"ok": True, "subscription": subscription, "proration": proration})
        except ValueError as e:
            return bad(handler, str(e))

    if parsed.path == "/api/billing/webhook":
        from api.billing import handle_billing_webhook
        result = handle_billing_webhook(body)
        return j(handler, result)

    # ── Payment API (POST) ──
    if parsed.path == "/api/billing/create_payment":
        from api.billing import create_payment_order, get_default_user_id
        try:
            require(body, "plan_id", "payment_method")
        except ValueError as e:
            return bad(handler, str(e))
        user_id = get_default_user_id()
        billing_cycle = body.get("billing_cycle", "yearly")
        try:
            result = create_payment_order(user_id, body["plan_id"], billing_cycle, body["payment_method"])
            # Auto-confirm for demo (in production, would wait for gateway callback)
            return j(handler, {"ok": True, "order_id": result["order_id"], "payment_url": result["payment_url"]})
        except ValueError as e:
            return bad(handler, str(e))

    # ── Recharge API (POST) ──
    if parsed.path == "/api/billing/recharge/create":
        from api.billing import create_recharge_order, get_default_user_id
        try:
            require(body, "amount_yuan", "payment_method")
        except ValueError as e:
            return bad(handler, str(e))
        user_id = get_default_user_id()
        try:
            result = create_recharge_order(user_id, int(body["amount_yuan"]), body["payment_method"])
            return j(handler, {"ok": True, "order_id": result["order_id"], "amount_yuan": result["amount_yuan"], "payment_url": result["payment_url"]})
        except ValueError as e:
            return bad(handler, str(e))

    if parsed.path == "/api/billing/recharge/webhook":
        from api.billing import recharge_webhook
        result = recharge_webhook(body)
        return j(handler, result)

    # ── Asset Library (POST) ──────────────────────────────────────────────────

    if parsed.path == "/api/assets/add":
        return _handle_add_asset(handler, body)

    if parsed.path == "/api/assets/remove":
        return _handle_remove_asset(handler, body)

    if parsed.path == "/api/assets/copy":
        return _handle_copy_asset(handler, body)

    # ── Chat Task Files (POST) ────────────────────────────────────────────────
    if parsed.path == "/api/chat-files":
        return handle_add_chat_file(handler, body)

    if parsed.path == "/api/chat-files/remove":
        return handle_remove_chat_file(handler, body)

    if parsed.path == "/api/chat-files/forward":
        return handle_forward_chat_file(handler, body)

    if parsed.path == "/api/chat-files/add-to-assets":
        return handle_add_to_assets(handler, body)
...
ASSETS_INDEX_FILE = STATE_DIR / "assets_index.json"
...
def _load_assets_index() -> dict:
    ...
    if ASSETS_INDEX_FILE.exists():
        try:
            _assets_cache = json.loads(ASSETS_INDEX_FILE.read_text(encoding="utf-8"))
        except Exception:
...
def _save_assets_index():
    global _assets_cache
    ASSET_DIR.mkdir(parents=True, exist_ok=True)
    ASSETS_INDEX_FILE.write_text(json.dumps(_assets_cache, ensure_ascii=False, indent=2), encoding="utf-8")
...
def _asset_file(name: str) -> Path:
    ...
    return ASSET_DIR / safe


def _handle_list_assets(handler):
    idx = _load_assets_index()
    assets = idx.get("assets", [])
    # Enrich with file info
    result = []
    for a in assets:
        fpath = _asset_file(a["name"])
        a_copy = dict(a)
        if fpath.exists():
            stat = fpath.stat()
            a_copy["size"] = stat.st_size
            a_copy["exists"] = True
            a_copy["modified"] = stat.st_mtime
        else:
            a_copy["exists"] = False
        result.append(a_copy)
    return j(handler, {"assets": result})


def _handle_get_asset(handler, name):
    fpath = _asset_file(name)
    if not fpath.exists():
    ...
    handler.end_headers()
    handler.wfile.write(fpath.read_bytes())
    return True


def _handle_list_workspace_files(handler, parsed):
    """List files in the default workspace for asset picker (no session required)."""
    ...
    try:
        entries = list_dir(base, path) if path == "." else list_dir(target.parent, target.name)
    except Exception:
        entries = []
    # Filter out directories
    file_entries = [e for e in entries if e["type"] != "dir"]
    return j(handler, {"entries": file_entries})


def _handle_add_asset(handler, body):
    """Copy a file from workspace to asset library."""
    source_path = body.get("source_path", "")
    name = body.get("name", "")
    if not source_path or not name:
        return bad(handler, "source_path and name required", 400)
    # Resolve workspace-relative path
    workspace = body.get("workspace", str(DEFAULT_WORKSPACE))
    # source_path may be a bare filename or a relative path like "subdir/file.md"
    clean_path = source_path.strip("/").lstrip("./")
    full_source = Path(workspace).resolve() / clean_path
    if not full_source.exists():
        return bad(handler, f"Source not found: {source_path}", 404)
    if full_source.is_dir():
        return bad(handler, "Can only add files, not directories", 400)
    try:
        dst = _asset_file(name)
    except ValueError as e:
        return bad(handler, str(e), 400)
    import shutil
    shutil.copy2(str(full_source), str(dst))
    with _ASSETS_LOCK:
        idx = _load_assets_index()
        # Check for duplicate
        for a in idx.get("assets", []):
            if a["name"] == name:
                return bad(handler, f"Asset already exists: {name}", 409)
        idx.setdefault("assets", []).append({
            "name": name,
...
def _handle_remove_asset(handler, body):
    ...
    with _ASSETS_LOCK:
        idx = _load_assets_index()
        idx["assets"] = [a for a in idx.get("assets", []) if a["name"] != name]
        _save_assets_index()
    if exists:
        fpath.unlink()
    return j(handler, {"ok": True, "removed": name})


def _handle_copy_asset(handler, body):
    """Copy an asset to the workspace (so it can be used as a file)."""
    name = body.get("name", "")
    workspace = body.get("workspace", str(DEFAULT_WORKSPACE))
    if not name:
        return bad(handler, "name required", 400)
    src = _asset_file(name)
    if not src.exists():
        return bad(handler, "Asset not found", 404)
    dst = Path(workspace).resolve() / name
    if dst.exists():
        return bad(handler, "File already exists in workspace", 409)
    import shutil
    shutil.copy2(str(src), str(dst))
    return j(handler, {"ok": True, "path": str(dst.relative_to(Path(workspace).resolve()))})
...
def _handle_sse_stream(handler, parsed):
    ...
    if q is None:
        _dlog("H1", "api/routes.py:1391", "SSE stream requested but not found", {"stream_id": stream_id})
        return j(handler, {"error": "stream not found"}, status=404)
    _dlog("H1", "api/routes.py:1393", "SSE stream opened", {"stream_id": stream_id, "client": handler.client_address[0]})
    handler.send_response(200)
        ...
        while True:
            try:
                ...
                try:
                    ...
                    # Extra flush after terminal events to ensure the client
                    # fully processes the event before the TCP connection closes.
                    # Without this, the EventSource 'error' event can fire
                    # before the apperror data is parsed, overwriting the
                    # real error with "Connection lost".
                    ...
                    last_event_time = time.time()
                    if event in ("done", "error", "cancel", "apperror"):
                        _dlog("H2", "api/routes.py:1414", "SSE terminal event sent", {"stream_id": stream_id, "event": event})
                except (BrokenPipeError, ConnectionResetError, ConnectionAbortedError):
                    # Client disconnected, exit gracefully
                    _dlog("H2", "api/routes.py:1416", "SSE client disconnected while sending event", {"stream_id": stream_id, "event": event})
                    break
                ...
                try:
                    ...
                    _dlog("H2", "api/routes.py:1431", "SSE client disconnected during keepalive", {"stream_id": stream_id})
                    break
...
def _handle_chat_start(handler, body):
    ...
    # Billing: check usage limits before processing
    from api.billing import increment_prompt_count, get_default_user_id, get_plan, get_user_subscription
    user_id = get_default_user_id()
    subscription = get_user_subscription(user_id)
    if subscription:
        current_count, limit, exceeded = increment_prompt_count(user_id)
        plan = get_plan(subscription["plan_id"])
        if exceeded:
            return j(
                handler,
                {
                    "error": "BILLING_003: Usage limit exceeded",
                    "code": "BILLING_003",
                    "message": f"您本月的对话次数已用完({limit}次)，请升级套餐或等待下月重置",
    ...
    # ── Profile switch: ensure config is loaded from the session's profile ──────
    # When the session belongs to a non-default profile, switch the active profile
    # before resolving model/provider so resolve_model_provider reads the correct
    # config.yaml (api_key, base_url, etc.).
    session_profile = getattr(s, 'profile', None) or s.model  # model used as profile hint
    profile_switched = False
    try:
        from api.profiles import get_active_profile_name, switch_profile
        active = get_active_profile_name()
        if session_profile and session_profile != active:
            try:
                switch_profile(session_profile)
                profile_switched = True
            except (ValueError, RuntimeError):
                # Profile doesn't exist or agent busy — fall through with active profile
                logger.warning(
                    "Cannot switch to profile '%s' for session %s: falling back to active profile '%s'",
                    session_profile, s.session_id, active,
                )
    except ImportError:
        logger.debug("profiles module not available for profile switching")
    ...
    # If profile was switched but the session still has a stale model from the old
    # profile, try to resolve the correct model from the new profile's config.
    if profile_switched and model:
        resolved_model, resolved_provider, resolved_base_url = resolve_model_provider(model)
        if resolved_provider or resolved_base_url:
            # Model resolved correctly with the new profile's config — use it as-is.
            pass
        else:
            # The session's model didn't resolve (e.g. it was a model name from a
            # different profile). Try to get the profile's configured default model.
            from api.config import get_config as _gc
            _cfg = _gc()
            _model_cfg = _cfg.get('model', {})
            if isinstance(_model_cfg, dict):
                profile_default = _model_cfg.get('default')
                if profile_default:
                    model = profile_default
                    logger.info(
                        "Session %s has stale model %r; using profile default %r",
                        s.session_id, model, profile_default,
                    )
    _dlog(
        "H1",
        "api/routes.py:1955",
        "Chat start received",
        {
            ...
            "model": str(model or ""),
            "workspace": str(body.get("workspace") or s.workspace or ""),
    ...
    if current_stream_id:
        ...
        if current_active:
            _dlog("H1", "api/routes.py:1964", "Rejected chat start: active stream exists", {"session_id": s.session_id, "active_stream_id": current_stream_id})
            return j(
                handler,
                {
    ...
    with STREAMS_LOCK:
        STREAMS[stream_id] = q
    _dlog("H1", "api/routes.py:1984", "Stream queue created", {"session_id": s.session_id, "stream_id": stream_id})
    ...
    thr.start()
    _dlog("H1", "api/routes.py:1991", "Agent thread started", {"session_id": s.session_id, "stream_id": stream_id})
...
def _handle_gitea_save(handler, body):
    """Save Gitea connection settings."""
    from api.config import save_gitea_settings

    gitea_url = body.get("gitea_url", "http://localhost:3000").strip()
    gitea_username = body.get("gitea_username", "").strip()
    gitea_token = body.get("gitea_token", "").strip()

    try:
        result = save_gitea_settings(gitea_url, gitea_username, gitea_token)
        return j(handler, result)
    except ValueError as e:
        return bad(handler, str(e))


def _handle_gitea_repos(handler):
    """List repositories from Gitea API."""
    from api.config import load_gitea_settings
    import urllib.request
    import json as _json

    settings = load_gitea_settings()
    if not settings.get("gitea_connected"):
        return bad(handler, "Not connected to Gitea", 401)
    ...
    try:
        # Use token as password for HTTP Basic Auth
        import base64
        auth = base64.b64encode(f"{username}:{token}".encode()).decode()
        req = urllib.request.Request(
            f"{gitea_url}/api/v1/user/repos?sort=updated&per_page=50",
            headers={
        ...
        return bad(handler, f"Gitea API error: {e.code} {e.reason}", e.code)
    except Exception as e:
        return bad(handler, f"Failed to fetch repos: {e}")


def _handle_gitea_clone(handler, body):
    """Clone a Gitea repository to local workspace directory."""
    from api.workspace import load_workspaces, save_workspaces

    repo_url = body.get("clone_url", "").strip()
    repo_name = body.get("name", "").strip()
    if not repo_url:
        return bad(handler, "clone_url is required")

    # Determine target directory
    try:
        workspaces = load_workspaces()
        default_ws = workspaces[0]["path"] if workspaces else str(Path.home() / "workspace")
        target_dir = Path(default_ws).resolve()
    except Exception:
    ...
    if clone_path.exists():
        return bad(handler, f"Directory already exists: {repo_name}")

    try:
        import subprocess

        result = subprocess.run(
            ["git", "clone", "--recursive", repo_url, str(clone_path)],
            capture_output=True,
            text=True,
            timeout=120,
        )
        if result.returncode != 0:
            return bad(handler, f"Git clone failed: {result.stderr}")

        # Add as a workspace
        wss = load_workspaces()
        if not any(w["path"] == str(clone_path) for w in wss):
            wss.append({"path": str(clone_path), "name": repo_name})
            save_workspaces(wss)
        ...
        return bad(handler, "Clone timed out (repo may be large)")
    except FileNotFoundError:
        return bad(handler, "Git is not installed")
    except Exception as e:
        return bad(handler, f"Clone failed: {e}")
